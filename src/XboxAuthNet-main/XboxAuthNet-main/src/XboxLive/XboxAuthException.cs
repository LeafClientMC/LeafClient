// File: XboxAuthNet/XboxLive/XboxAuthException.cs (in base XboxAuthNet project)
using System;
using System.Linq;
...
namespace XboxAuthNet.XboxLive
{
    public class XboxAuthException : Exception
    {
        ...
        public XboxAuthException(string? error, string? message, string? redirect, int statusCode)
            : base(CreateMessageFromError(error, message, redirect)) =>
        ...
        public static XboxAuthException FromResponseBody(string responseBody, int statusCode)
        {
            try
            {
                // CRITICAL: Use the AOT-safe options
                var options = XboxAuthNet.JsonConfig.DefaultOptions ?? new JsonSerializerOptions();
                var errRes = JsonSerializer.Deserialize<XboxErrorResponse>(responseBody, options);
