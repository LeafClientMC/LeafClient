...
public class JsonSessionStorage : ISessionStorage
{
    ...
    {
        // Ensure that if jsonOptions is default, we use the global AOT-safe options
        var effectiveOptions = jsonOptions ?? XboxAuthNet.Game.JsonConfig.DefaultOptions ?? new JsonSerializerOptions();
        return new JsonSessionStorage(new JsonObject(), effectiveOptions);
    }
    ...
    {
        // Ensure _jsonOptions is always set to an AOT-safe resolver, falling back to global config
        this._jsonOptions = jsonSerializerOptions ?? XboxAuthNet.Game.JsonConfig.DefaultOptions ?? new JsonSerializerOptions();
    ...
    public bool TryGetValue<T>(string key, out T value)
    {
        ...
        {
            value = default!; // Ensure value is assigned in all paths
            return false;
        }
    }
    ...
    private bool getFromJson<T>(string key, out T value)
    {
        ...
        {
            // Use the _jsonOptions field, which is guaranteed to be AOT-safe
