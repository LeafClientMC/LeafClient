...
public class XboxUnsignedDeviceTokenAuth : SessionAuthenticator<XboxAuthTokens>
{
    ...
    {
        LeafClient.Services.LeafLog.Info("XboxUnsignedDeviceTokenAuth", $"Starting unsigned device token authentication...");
        ...
        try
        {
            ...
            if (response.IsSuccessStatusCode)
            {
                ...
                LeafClient.Services.LeafLog.Info("XboxUnsignedDeviceTokenAuth", $"Device token obtained successfully");
            }
            else
            {
                LeafClient.Services.LeafLog.Info("XboxUnsignedDeviceTokenAuth", $"ERROR: {response.StatusCode} - {responseBody}");
                throw new Exception($"Device token request failed: {response.StatusCode}");
            }
        }
        catch (Exception ex)
        {
            LeafClient.Services.LeafLog.Info("XboxUnsignedDeviceTokenAuth", $"ERROR: {ex.Message}");
            throw;
        }
