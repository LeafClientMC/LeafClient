...
namespace LeafClient.PrivateServices
{
    ...
    internal class SuggestionsService
    {
        ...
        private async Task SaveFeedbackLocallyAsync<T>(T payload, string typeSuffix) where T : class
        {
            ...
            {
                Console.Error.WriteLine($"[SuggestionsService] Failed to save feedback locally: {ex.Message}");
            }
        }

        /// <summary>
        /// Handles the actual HTTP POST request to the worker.
        /// Always saves the payload locally first so data is never lost if the send fails.
        /// </summary>
