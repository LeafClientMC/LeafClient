...
namespace LeafClient.PrivateServices
{
    public class OnlineCountService : IDisposable
    {
        ...
        public string? GetMotdBgColor()
        {
            return _cachedConfig?.MotdBgColor;
        }

        public string? GetMotdLink()
        {
            return _cachedConfig?.MotdLink;
        }
        ...
        private async Task RefreshConfigAsync()
        {
            ...
            try
            {
                ...
                {
                    LeafLog.Info("OnlineCountService", "WARN: GetConfigAsync returned null");
                }
            }
            catch (Exception ex)
            {
                LeafLog.Info("OnlineCountService", $"RefreshConfigAsync error: {ex.Message}");
            }
            finally
            {
