                ...
                try
                {
                            ...
                            CRYPTPROTECT_UI_FORBIDDEN, ref outBlob))
                    {
                        ...
                        throw new CryptographicException(
                            $"CryptProtectData failed (Win32 error 0x{err:X8} / {err}). " +
                            "User profile may be corrupt or roaming-unavailable.");
                    }
            ...
            public static byte[] Unprotect(byte[] cipher)
            {
                ...
                try
                {
                            ...
                            CRYPTPROTECT_UI_FORBIDDEN, ref outBlob))
                    {
                        int err = Marshal.GetLastWin32Error();
                        throw new CryptographicException(
                            $"CryptUnprotectData failed (Win32 error 0x{err:X8} / {err}). " +
                            "Credentials may have been encrypted by a different Windows user account.");
                    }
                    var result = new byte[outBlob.cbData];
                    Marshal.Copy(outBlob.pbData, result, 0, outBlob.cbData);
                    return result;
        ...
        private static class AesGcmFallback
        {
            ...
            private static byte[] DeriveKey(byte[] salt)
            {
                ...
                using var pbkdf2 = new Rfc2898DeriveBytes(
                    Encoding.UTF8.GetBytes(machineId), salt, 100_000, HashAlgorithmName.SHA256);
            ...
            {
                if (cipher.Length < 1) throw new CryptographicException("Cipher too short");
                int version = cipher[0];

                if (version == Version)
                {
                    if (cipher.Length < 1 + SaltSize + NonceSize + TagSize)
                        throw new CryptographicException("Cipher too short for v2");
                    byte[] salt = new byte[SaltSize];
                    byte[] nonce = new byte[NonceSize];
                    byte[] tag = new byte[TagSize];
                    byte[] ciphertext = new byte[cipher.Length - 1 - SaltSize - NonceSize - TagSize];
                    Buffer.BlockCopy(cipher, 1, salt, 0, SaltSize);
                    Buffer.BlockCopy(cipher, 1 + SaltSize, nonce, 0, NonceSize);
                    Buffer.BlockCopy(cipher, 1 + SaltSize + NonceSize, tag, 0, TagSize);
                    Buffer.BlockCopy(cipher, 1 + SaltSize + NonceSize + TagSize, ciphertext, 0, ciphertext.Length);
                ...
                if (version == LegacyVersion)
                {
                    if (cipher.Length < 1 + NonceSize + TagSize)
                        throw new CryptographicException("Cipher too short for v1");
                    byte[] nonce = new byte[NonceSize];
                    byte[] tag = new byte[TagSize];
                ...
                throw new CryptographicException("Unsupported credential version: " + version);
            }
        }
    }
}
