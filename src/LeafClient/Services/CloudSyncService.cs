    ...
    public static class CloudSyncService
    {
        ...
        private static JsonElement? SerializeLauncherSettings(LauncherSettings settings)
        {
            try
            {
                ...
                {
                    string activeId = clone.ActiveAccountId ?? "";
                    foreach (var acc in clone.SavedAccounts)
                    {
                        acc.AccessToken = null;
                        acc.LeafApiJwt = null;
                        acc.LeafApiRefreshToken = null;
                        if (!string.Equals(acc.Id, activeId, StringComparison.Ordinal))
                        {
                            acc.Equipped = new EquippedCosmetics();
                            acc.OwnedCosmeticIds = new List<string>();
                            acc.CosmeticPresets = new List<CosmeticPreset>();
                        }
                    }
                }
            ...
            {
                LeafLog.Info("CloudSync", $"SerializeLauncherSettings failed: {ex.Message}");
                return null;
            }
        }
        ...
        private static JsonElement? ReadModSettings()
        {
            ...
            {
                LeafLog.Info("CloudSync", $"ReadModSettings failed: {ex.Message}");
                return null;
            }
        }
        ...
        private static JsonElement? ReadServerList()
        {
            ...
            {
                LeafLog.Info("CloudSync", $"ReadServerList failed: {ex.Message}");
                return null;
            }
        }
        ...
        private static JsonElement? ReadResourcePacks()
        {
            ...
            {
                LeafLog.Info("CloudSync", $"ReadResourcePacks failed: {ex.Message}");
                return null;
            }
        }
        ...
        public static void ApplyRemoteAuxiliary(LeafApiSyncPullResult remote)
        {
            ...
            catch (Exception ex) { LeafLog.Info("CloudSync", $"Apply mod_settings failed: {ex.Message}"); }
            ...
            catch (Exception ex) { LeafLog.Info("CloudSync", $"Apply server_list failed: {ex.Message}"); }
            ...
            catch (Exception ex) { LeafLog.Info("CloudSync", $"Apply resource_packs failed: {ex.Message}"); }
        }
