...
public class FabricInstaller
{
    ...
    public async Task<IReadOnlyCollection<string>> GetSupportedVersionNames()
    {
        ...
        var list = await JsonSerializer.DeserializeAsync(
            res,
            CmlLibJsonContext.Default.IEnumerableFabricLoader);
        if (list == null)
    ...
    public async Task<IReadOnlyCollection<FabricLoader>> GetLoaders()
    {
        ...
        var list = await JsonSerializer.DeserializeAsync(
            res,
            CmlLibJsonContext.Default.IReadOnlyCollectionFabricLoader);
        if (list == null)
    ...
    private IEnumerable<FabricLoader> parseLoaders(JsonElement root)
    {
        ...
        foreach (var item in root.EnumerateArray())
        {
            ...
            var loader = loaderElement.Value.Deserialize<FabricLoader>(new JsonSerializerOptions
            {
