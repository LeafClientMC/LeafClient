...
public class MinecraftJavaManifestResolver
{
    ...
    {
        // Fix: Use source-generated serialization instead of reflection
        MFileMetadata? metadata = null;
        var manifestElement = json.GetPropertyOrNull("manifest");
        if (manifestElement != null)
        {
            metadata = manifestElement.Value.Deserialize(
                CmlLibJsonContext.Default.MFileMetadata);
        }
